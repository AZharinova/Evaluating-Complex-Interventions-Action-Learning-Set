...
DTOC %>% 
  ...
  table() %>% 
  prop.table(margin=2) %>% 
  apply(MARGIN=2, FUN=scales::percent_format(accuracy=0.1)) %>% 
...
# Combine data
did_plotdata <- bind_rows(differences, DTOC_counter)

did_plotdata %>% 
  mutate(label=if_else(Month=="Jan-20", as.character(Region), NA_character_)) %>% 
  ggplot(aes(x=Month, y=DTOC_days, group=Region)) +
  geom_line(aes(color=Region)) + scale_y_continuous(limits=c(300, 900))
